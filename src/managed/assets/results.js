...
async function refresh() {
  if (!key || !config || loading) return;
  loading = true; const requestKey = key, days = $('days').value, cohort = $('cohort').value;
  $('status').textContent = 'Loading saved results…'; $('refresh').disabled = true;
  try {
    const response = await fetch(config.apiBase+'/report?days='+days+'&cohort='+cohort,{headers:{Authorization:'Bearer '+requestKey},cache:'no-store'});
    const data = await response.json(); if (!response.ok) throw new Error(data.error || 'Could not load results.');
    if (key === requestKey && days === $('days').value && cohort === $('cohort').value) render(data);
  } catch(error) { if (key === requestKey) { latest = undefined; ready(false); $('status').textContent = error.message; $('status').className = 'error'; } }
  finally { loading = false; $('refresh').disabled = false; if (key && (key !== requestKey || days !== $('days').value || cohort !== $('cohort').value)) void refresh(); }
}
function lock() { key = ''; latest = undefined; ready(false); $('key').value = ''; $('status').textContent = 'Results locked. Enter your key to reopen.'; $('status').className = ''; }
$('login').onsubmit = (e) => { e.preventDefault(); key = $('key').value.trim(); $('key').value = ''; void refresh(); };
$('refresh').onclick = refresh;
for (const id of ['days','cohort']) $(id).onchange = () => { latest = undefined; $('report').hidden = true; $('export').hidden = true; $('audience-badge').hidden = true; void refresh(); };
$('lock').onclick = lock; $('lock-top').onclick = lock;
$('export').onclick = () => { if (!latest) return; const url = URL.createObjectURL(new Blob([JSON.stringify(latest,null,2)],{type:'application/json'})), a = node('a'); a.href = url; a.download = config.project+'-'+latest.cohort+'.json'; a.click(); setTimeout(() => URL.revokeObjectURL(url),1000); };
...
try {
  const response = await fetch('/__launchlab/config.json',{cache:'no-store'}); if (!response.ok) throw new Error('Experiment settings are unavailable. Reload to try again.'); config = await response.json();
  $('name').textContent = config.name; $('project-name').textContent = config.name; $('hypothesis').textContent = config.hypothesis;
  document.querySelector('.project-mark').textContent = config.name.charAt(0).toUpperCase();
  $('revision').textContent = config.experiment?.sourceCommit ? 'Version '+config.experiment.sourceCommit.slice(0,7) : 'Product test';
} catch(error) { $('status').textContent = error.message; $('status').className = 'error'; $('login').querySelector('button').disabled = true; }
