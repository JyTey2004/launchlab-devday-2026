// LaunchLab — From code to learning. Native Higgsedit, 176 seconds.
// Product motion is an illustrated reconstruction; photographs and captures are labelled.
import fs from 'node:fs';
export default async ({project}) => {
 const base=process.env.LAUNCHLAB_FILM_DIR||'/home/user/launchlab-film-v3';
 ...
 const chrome=(index,kind='Illustrated workflow',dark=false)=>[...brand(dark),T(kind,1010,56,825,30,18,dark?'#BBAACD':C.muted,400,{align:'right'}),line(80,118,1760,dark?'#34233F':C.line),T('launchlab.stardive.xyz  /  OKX DEV DAY 2026',80,1027,1220,28,16,dark?'#A591B9':C.muted,500,{letterSpacing:1.6}),T(index,1700,1027,140,28,18,dark?'#A591B9':C.muted,400,{align:'right'})];
 const scene=(at,dur,name,nodes,{dark=false,kind='Illustrated workflow',header=true,fixed=false}={})=>p.compose(F(0,0,1920,1080,[...(dark?darkBg:lightBg),...(header?chrome(String(at).padStart(3,'0'),kind,dark):[]),...nodes]),{at:at>=25&&!fixed?at+8:at,dur,name});
 ...
 scene(19,6,'LaunchLab brand reveal',[
   I('logo.png',860,208,200,200,{fit:'contain'}),T('LaunchLab.',130,449,1660,155,124,'#FAF6FF',500,{align:'center',motion:{by:'word',from:{opacity:0,y:22},at:.3,duration:.6,overlap:.65,easing:'house'}}),T('From code to a testable product.',130,638,1660,85,44,C.lilac,400,{align:'center'}),F(420,811,1080,60,T('launchlab.stardive.xyz',0,0,1080,60,23,'#B4A3C6',500,{align:'center',letterSpacing:5}),enter(1.9))
 ],{dark:true,header:false});

 // Main destination: an editable, website-inspired view based on the observed live homepage.
 scene(25,8,'Explore LaunchLab at its main website',[
   label('START HERE / THE LAUNCHLAB WEBSITE',80,155,1740,C.lilac),
   F(80,224,1760,712,[R(0,0,1760,712,'#0B0A0E',24,{strokeColor:'#554263',strokeWidth:1.5}),
     R(0,0,1760,60,'#211729',24),R(0,31,1760,29,'#211729'),
     ...[0,1,2].map(i=>R(23+i*26,23,11,11,['#EEA2BB','#DDC090','#A8C5B5'][i],6)),
     R(472,12,816,36,'#100C17',9),T('launchlab.stardive.xyz',488,21,784,27,20,'#D9C8E7',500,{align:'center'}),
     ...Array.from({length:21},(_,i)=>R(20+i*86,82,1,598,'#231C2B')),
     ...Array.from({length:7},(_,i)=>R(20,82+i*86,1720,1,'#231C2B')),
     I('logo.png',52,83,36,36,{fit:'contain'}),T('launchlab.',103,85,490,44,28,'#FAF6FF',500),T('The launch loop     Use cases     Why OKX',900,95,792,34,18,'#B5A6C2',400,{align:'right'}),
     label('YOUR LAUNCH AGENT, AS A SERVICE',56,214,850,'#AA96BD'),
     T('Launch fast.',50,274,1000,116,86,'#FAF6FF',500),T('Learn faster.',50,380,1000,116,86,C.lilac,500),
     T('Give your agent a repo and a goal. Connect launch,\nfocused tests, and useful feedback in one repeatable workflow.',56,514,880,87,26,'#B8ACCA'),
     R(56,623,273,58,'#DCC9EE',10),T('Plan your launch',66,641,253,34,23,'#231A31',500,{align:'center'}),T('Brief your agent',377,643,350,35,22,'#D3C3E0'),
     ...[0,1,2,3,4].map((i)=>F(1070,478-i*47,590,240,[
        <path width={590} height={240} d="M0 91 L302 0 L590 87 L288 196 Z" fill={i===4?'#30213D':'#201729'} stroke={{color:'#6B507E',width:1.2}}/>,
        <path width={590} height={240} d="M0 91 L288 196 L590 87 V104 L288 213 L0 108 Z" fill={i===4?'#815BA8':'#4B315F'}/>,
        T(['Source','Preview','Missions','Feedback','Agent'][i],239,147,205,32,20,'#D8C3E9',400,{rotation:-18})
     ],{...enter(.2+i*.2,50,20)}))
   ],enter(.1,0,35)),
   T('Understand the launch loop. Explore use cases. Prepare your brief.',80,966,1760,54,28,'#C7B4D7',400,{align:'center'}),
   T('Animated editorial view of the live homepage',80,1031,1760,30,17,'#A791BA',400,{align:'right'})
 ],{dark:true,header:false,fixed:true});
 ...
 scene(158,10,'LaunchLab: ship, observe, learn',[
   I('logo.png',94,100,78,78,{fit:'contain'}),T('launchlab.',195,113,1350,74,53,'#FAF6FF',500),hero('Ship something\npeople can try.',89,283,1730,266,108),F(90,612,1700,92,T('Learn what to build next.',0,0,1700,82,51,C.lilac),enter(1.4)),T('Start here. Explore what LaunchLab can do.',94,796,1700,48,29,'#D2C3E0'),T('launchlab.stardive.xyz',94,848,1700,92,58,'#FAF6FF',500),T('Tey Jia Ye  /  Build a Company  /  OKX Dev Day 2026',94,944,1700,32,20,'#AB96BE'),T('Music: “Electric Dreams” — Scott Buckley · CC BY 4.0 · Edited excerpt\nscottbuckley.com.au · creativecommons.org/licenses/by/4.0',94,991,1700,65,17,'#AB96BE')
 ],{dark:true,header:false});
 const times=[3,8.8,16.5,22,28,31,37.5,47,57.2,67,76,79,88,95,99,107,116,120,128,141,149,160,171];
 ...
 console.log(JSON.stringify({duration:176,frames:times.length,project:base+'/project',render:process.env.LAUNCHLAB_RENDER==='1'}));
};
